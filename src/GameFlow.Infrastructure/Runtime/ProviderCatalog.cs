...
public static class ProviderCatalog
{
    ...
    [
        ...
        new ProviderIdentity("hidmaestro",    "HIDMaestro",          "Windows virtual HID output",   true, "User-mode virtual controller platform (UMDF2 — no kernel driver or reboot). Presents as real hardware to XInput, DirectInput, SDL3 and WGI. Activates automatically if HIDMaestro.Core.dll is placed next to the executable, or in builds compiled with the HIDMaestro SDK. Does not fall back to ViGEm — if HIDMaestro can't activate, this slot has no output (select a ViGEm provider directly if you want one)."),
    ];
}
